...
#signal astar_path_calculated(points: Array[Vector2i], moving_unit: BattleUnit)
#
...
#func initialize(battle_grid_node: UnitGrid, game_area_node: PlayArea) -> void:
	...
	#
	## Use Vector2i size since we're only using X and Z axes
	#full_grid_region = Rect2i(Vector2i.ZERO, battle_grid.size)
	#astar_grid = AStarGrid2D.new()
	#astar_grid.region = full_grid_region
	#astar_grid.cell_size = Vector2(game_area.cell_size.x, game_area.cell_size.z)  # Use X and Z from GridMap
...
#func update_occupied_tiles() -> void:
	...
	#for tile_pos: Vector2i in battle_grid.get_all_ocuppied_tiles():
		#astar_grid.set_point_solid(tile_pos)
#
#
#func get_next_position(moving_unit: BattleUnit, target_unit: BattleUnit) -> Vector3:
	...
	## we stay there and add it back as a solid
		...
		#return Vector3(-1, 0, -1)  # Invalid position indicator for 3D
	#
