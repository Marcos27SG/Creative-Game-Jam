class_name CrisisDecision
extends Control
@export var crisis_pool: Array[Decision]
...
func _ready() -> void:
	pass
	Events.crisis_choice_triggered.connect(show_new_strategy)
...
func show_strategy_decision(decision_day) -> void:
	#self.show()
	print(decision_day)
	var decision = crisis_pool[decision_day] 
